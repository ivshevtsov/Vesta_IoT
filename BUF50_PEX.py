...
HOME = 'DATA/BUF50_PEX/csv'
File_PVT = 'buf50_pvt_pex_plot.csv'

PVT = genfromtxt(f'{HOME}/{File_PVT}', delimiter=',', skip_header=1)

# plot S21
plt.figure()
plot_csv_column(PVT, column_x=3, column_y=4, xlabel='f, Гц', ylabel='S21, дБ', percent=100)
plt.xscale('log')
plt.grid()

# plot S22
plt.figure()
plot_csv_column(PVT, column_x=5, column_y=6, xlabel='f, Гц', ylabel='S22, дБ', percent=100)
plt.xscale('log')
plt.grid()
