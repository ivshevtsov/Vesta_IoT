...
#input transistors
L_in = 2e-6
gm_in, W_in, r_out_in = gm_W(Id=90e-6, Veff=0.25,L=L_in, Type="P")
fc_in, thermal_in = flicker_thermal(Id=90e-6, Veff=0.25,L=L_in, Type="P")
print_par(Title='#Input transistor#',gm=gm_in, W=W_in, L=L_in, rout=r_out_in, fc=fc_in, thermal_v=thermal_in)

#load transistors
L_load = 3e-6
gm_load, W_load, r_out_load = gm_W(Id=90e-6, Veff=0.25,L=L_load, Type="N")
fc_load, thermal_load = flicker_thermal(Id=90e-6, Veff=0.25,L=L_load, Type="N")
print_par(Title='#Load transistor#',gm=gm_load, W=W_load, L=L_load, rout=r_out_load, fc=fc_load, thermal_v=thermal_load)

#tail transistor
L_tail = 2e-6
gm_tail, W_tail, r_out_tail = gm_W(Id=180e-6, Veff=0.25,L=L_tail, Type="P")
fc_tail, thermal_tail = flicker_thermal(Id=180e-6, Veff=0.25,L=L_tail, Type="P")
