...
HOME = 'DATA/LNA_PEX/csv'
File_PVT = 'lna_gnss_pvt_plot.csv'

PVT = genfromtxt(f'{HOME}/{File_PVT}', delimiter=',', skip_header=1)

# plot S11
plt.figure()
plot_csv_column(PVT, column_x=3, column_y=4, xlabel='f, Гц', ylabel='S11, дБ', percent=100)
#plt.xscale('log')
plt.grid()

# plot S21
plt.figure()
plot_csv_column(PVT, column_x=5, column_y=6, xlabel='f, Гц', ylabel='Gain, дБ', percent=100)
#plt.xscale('log')
plt.grid()
